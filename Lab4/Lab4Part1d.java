 ...
 * @author marti
 */
public class Lab4Part1d 
{
    public static void main(String[] args) throws FileNotFoundException
    {
        String newFileName = "robotsInfoHW.txt";
        String newOutFile = "Lab4Part1d.out";
        String robotName; 
        int robotYear; 
        String robotKind; 
        char robotSource; 
        ...
        while(inputFile.hasNext())
        {
            ...
            Scanner delimiter = new Scanner(RobotString2D[count][iTitle]);
            delimiter.useDelimiter(",");
            RobotString2D[count][iTitle] = delimiter.next();
            
            robotHeight = delimiter.next();
            try
            {
            RobotDouble2D[count][iHeight] = Double.parseDouble(robotHeight);
            }
            catch (NumberFormatException nfe)
            {
                RobotString2D[count][iTitle] = RobotString2D[count][iTitle] + ", " + robotHeight;
                RobotDouble2D[count][iHeight] = Double.parseDouble(delimiter.next());
            }
            
            RobotDouble2D[count][iWeight] = Double.parseDouble(delimiter.next());
            RobotChar2D[count][iImplemented] = delimiter.next().charAt(1);
            if(delimiter.hasNext())
            {
                RobotString2D[count][iActor] = delimiter.next();
            }
            delimiter.close();
        ...
        for(int d = 0; d < count;d++)
        {
            for(int i = 1; i < (count-d);i++)
            {
                if(RobotInt2D[i-1][iCostValue] > RobotInt2D[i][iCostValue])
                {
                    ...
                    tempd = RobotDouble2D[i][iHeight];
                    RobotDouble2D[i][iHeight] = RobotDouble2D[i-1][iHeight];
                    RobotDouble2D[i-1][iHeight] = tempd;
                    
                    tempd = RobotDouble2D[i][iWeight];
                    RobotDouble2D[i][iWeight] = RobotDouble2D[i-1][iWeight];
                    RobotDouble2D[i-1][iWeight] = tempd;
                    
                    robotSource = RobotChar2D[i][iImplemented];
                    RobotChar2D[i][iImplemented] = RobotChar2D[i-1][iImplemented];
                    RobotChar2D[i-1][iImplemented] = robotSource;
                    
                    contain = RobotString2D[i][iActor];
                    RobotString2D[i][iActor] = RobotString2D[i-1][iActor];
                    RobotString2D[i-1][iActor] = contain;
                }
            }
        }
        System.out.println("");
    ...
    public static void RoboTable(String RobotString2D[][], int RobotInt2D[][], double RobotDouble2D[][], char RobotChar2D[][] , int count, String newOutFile) throws FileNotFoundException
    {
        ...
        String vheight = "HEIGHT";
        String vweight = "WEIGHT";
        String vImplemented = "IMPLEMENTED";
        String vactor = "ACTOR";
        ...
        for(int c = 0; c < (titlel-vtitle.length()+1);c++)
        {
            System.out.print(" ");
            OutFile.print(" ");
        }
        System.out.print(vdecimal);
        OutFile.print(vdecimal);
        for(int c = 0; c < 5;c++)
        {
            System.out.print(" ");
            OutFile.print(" ");
        }
        System.out.print(vheight);
        OutFile.print(vheight);
        for(int c = 0; c < 10;c++)
        {
            System.out.print(" ");
            OutFile.print(" ");
        }
        System.out.print(vweight);
        OutFile.print(vweight);
        for(int c = 0; c < 10;c++)
        {
            System.out.print(" ");
            OutFile.print(" ");
        }
        System.out.print(vImplemented);
        OutFile.print(vImplemented);
        for(int c = 0; c < 10;c++)
        {
            System.out.print(" ");
            OutFile.print(" ");
        ...
        for(int loc = 0; loc < count;loc++) //start for data inputs
        {
            ...
            System.out.print(" " +RobotString2D[loc][iTitle]);
            OutFile.print(" " +RobotString2D[loc][iTitle]);
            for(int c = 0;c < (titlel-RobotString2D[loc][iTitle].length()+1);c++)
            {
                System.out.print(" ");
                OutFile.print(" ");
            }
            System.out.print(" "+RobotDouble2D[loc][iCostd]);
            OutFile.print(" "+RobotDouble2D[loc][iCostd]);
            for(int c = 0; c < 9;c++)
            {
                System.out.print(" ");
                OutFile.print(" ");
            }
            System.out.print(RobotDouble2D[loc][iHeight]);
            OutFile.print(RobotDouble2D[loc][iHeight]);
            for(int c = 0; c < 12;c++)
            {
                System.out.print(" ");
                OutFile.print(" ");
            }
            System.out.print(RobotDouble2D[loc][iWeight]);
            OutFile.print(RobotDouble2D[loc][iWeight]);
            for(int c = 0; c < 15;c++)
            {
                System.out.print(" ");
                OutFile.print(" ");
            }
            System.out.print(RobotChar2D[loc][iImplemented]);
            OutFile.print(RobotChar2D[loc][iImplemented]);
            for(int c = 0; c < 10;c++)
            {
                System.out.print(" ");
                OutFile.print(" ");
            }
            System.out.println(RobotString2D[loc][iActor]);
            OutFile.println(RobotString2D[loc][iActor]);
        }
